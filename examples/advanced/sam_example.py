...
def create_model():
    ...
    for i in [0, 3, 6]:  # Linear layers
        ...
        model.layers[i].weight.data = (
            np.random.randn(*model.layers[i].weight.data.shape) * std
        )
...
def train_and_evaluate(
    model, train_loader, val_loader, optimizer_name, learning_rate=0.01, epochs=10
):
    """
...
def run_optimizer_comparison():
    ...
    for optimizer_name in optimizers:
        print(
            f"{optimizer_name:<10}: Test Accuracy = {results[optimizer_name]['test_accuracy']:.2%}"
        )
...
def plot_results(results, optimizers):
    ...
    for i, opt in enumerate(optimizers):
        ...
        plt.bar(
            x + i * width - (len(optimizers) - 1) * width / 2,
            class_acc,
            width,
            label=opt,
        )
