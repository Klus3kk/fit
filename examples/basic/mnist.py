...
def train_and_evaluate_mnist():
    ...
    for i, param in enumerate(model.parameters()):
        print(
            f"  Param {i}: shape {param.data.shape}, requires_grad={param.requires_grad}"
        )
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
            ...
            if batch_idx % 20 == 0:
                ...
                print(
                    f"Epoch {epoch+1}/{epochs}, Batch {batch_idx}, Loss: {loss.data:.4f}, Acc: {current_acc:.1f}%"
                )
        ...
        print(
            f"Epoch {epoch+1}/{epochs} completed. Average Loss: {avg_loss:.4f}, Accuracy: {epoch_accuracy:.2f}%"
        )

        # Test every 3 epochs
        if (epoch + 1) % 3 == 0:
            test_accuracy = evaluate_model(
                model, X_test_subset[:100], y_test_subset[:100]
            )
    ...
    for i in range(16):
        ...
        plt.title(
            f"Pred: {sample_predictions[i]}, True: {sample_labels[i]}", color=color
        )
        plt.axis("off")
