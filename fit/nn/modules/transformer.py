...
from attention_core import (
    MultiHeadAttention, SelfAttention, CrossAttention, CausalSelfAttention
)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        def _backward():
            ...
            if x.requires_grad:
                ...
                grad_norm_mean = np.mean(output.grad * normalized, axis=-1, keepdims=True)
...
class GELU(Layer):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        def _backward():
            ...
            derivative = (0.5 * (1 + tanh_output) + 
                         0.5 * x.data * sech2 * math.sqrt(2.0 / math.pi) * 
...
class TransformerEncoder(Layer):
    ...
    def __init__(
        ...
        for _ in range(num_layers):
            layer = TransformerEncoderBlock(d_model, num_heads, d_ff, dropout, activation)
...
class TransformerDecoder(Layer):
    ...
    def __init__(
        ...
        self.layers = []
        for _ in range(num_layers):
            layer = TransformerDecoderBlock(d_model, num_heads, d_ff, dropout, activation)
            self.layers.append(layer)
            self.add_child(layer)
...
class Embedding(Layer):
    ...
    def __init__(self, vocab_size: int, d_model: int):
        ...
        self.weight = Tensor(
            np.random.normal(0, scale, (vocab_size, d_model)), 
            requires_grad=True
        )
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        def _backward():
            ...
            self.weight.grad = (
                weight_grad if self.weight.grad is None 
...
class Transformer(Layer):
    ...
    def __init__(
        ...
        self.encoder = TransformerEncoder(
            ...
            d_ff, max_len, dropout, activation
        )
...
def demonstrate_transformer():
    ...
    print(f"✅ Positional Encoding: {dummy_embeddings.data.shape} -> {pos_output.data.shape}")
    
    # 2. Layer Normalization
    layer_norm = LayerNorm(d_model)
    norm_output = layer_norm(dummy_embeddings)
    print(f"✅ Layer Normalization: {dummy_embeddings.data.shape} -> {norm_output.data.shape}")
    ...
    print(f"✅ Encoder Block: {dummy_embeddings.data.shape} -> {block_output.data.shape}")
    print()
    ...
    encoder = TransformerEncoder(
        vocab_size=vocab_size, d_model=d_model, num_heads=4, 
        num_layers=2, d_ff=256, max_len=100
    )
    encoder_output = encoder(src_tokens)
    print(f"✅ Transformer Encoder: {src_tokens.data.shape} -> {encoder_output.data.shape}")
    
    # 2. Complete Transformer
    transformer = Transformer(
        src_vocab_size=vocab_size, tgt_vocab_size=vocab_size,
        d_model=d_model, num_heads=4, num_encoder_layers=2, 
        num_decoder_layers=2, d_ff=256, max_len=100
    )
    
    transformer_output = transformer(src_tokens, tgt_tokens)
    print(f"✅ Complete Transformer: {src_tokens.data.shape}, {tgt_tokens.data.shape} -> {transformer_output.data.shape}")
    print()
