...
class VotingClassifier:
        ...
        for name, estimator in self.estimators:
            ...
            if hasattr(cloned_estimator, 'fit'):
                ...
                try:
                    ...
                    train_data = (X, y)
                    tracker = train(cloned_estimator, train_data, epochs=50, verbose=False)
                except ImportError:
                    # Fallback if FIT modules not available
                    raise ValueError("Estimator must have a 'fit' method or be a FIT framework model")
    ...
    def _predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        all_probas = []
        weights = self.weights if self.weights is not None else [1.0] * len(self.estimators_)
            ...
            if hasattr(estimator, 'predict_proba'):
                ...
                try:
                    ...
                    if outputs.data.ndim > 1 and outputs.data.shape[1] > 1:
                        ...
                        probas = exp_outputs / np.sum(exp_outputs, axis=1, keepdims=True)
                    else:
