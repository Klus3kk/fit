...
class SimpleTrainer:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.val_loader = self._setup_data(validation_data, shuffle=False) if validation_data else None
    ...
    def _train_epoch(self):
        ...
        for x, y in self.train_loader:
            ...
            if 'accuracy' in self.metrics and outputs.data.ndim > 1 and outputs.data.shape[1] > 1:
                predictions = np.argmax(outputs.data, axis=1)
                targets = y.data.astype(np.int32) if y.data.ndim == 1 else np.argmax(y.data, axis=1)
...
def quick_evaluate(model, data: Union[tuple, DataLoader], metrics: List[str] = None) -> Dict[str, float]:
    """
