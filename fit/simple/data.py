...
def load_dataset(
    ...
    **kwargs
    ...
    if name.lower() == 'mnist':
        return _load_mnist(data_dir, batch_size, validation_split, shuffle, normalize, flatten)
    elif name.lower() == 'cifar10':
        return _load_cifar10(data_dir, batch_size, validation_split, shuffle, normalize, flatten)
    elif name.lower() == 'boston':
        return _load_boston(batch_size, validation_split, test_split, shuffle, normalize)
...
def _load_mnist(data_dir, batch_size, validation_split, shuffle, normalize, flatten):
    ...
    try:
        ...
        raise ImportError("sklearn required for MNIST. Install with: pip install scikit-learn")
...
def _load_cifar10(data_dir, batch_size, validation_split, shuffle, normalize, flatten):
    ...
    try:
        ...
        raise ImportError("sklearn required for CIFAR-10. Install with: pip install scikit-learn")
...
def _load_boston(batch_size, validation_split, test_split, shuffle, normalize):
    ...
    try:
        ...
        raise ImportError("sklearn required for Boston dataset. Install with: pip install scikit-learn")
...
def _load_iris(batch_size, validation_split, test_split, shuffle, normalize):
    ...
    try:
        ...
        raise ImportError("sklearn required for Iris dataset. Install with: pip install scikit-learn")
...
def _load_spiral(batch_size, shuffle, n_samples=1000, noise=0.1, n_spirals=2):
    ...
    for i in range(n_spirals):
        ...
        theta = np.linspace(i * 2 * np.pi / n_spirals, 
...
def load_tiny(dataset_name: str, max_samples: int = 1000, **kwargs):
    ...
    for split_name, loader in data.items():
        if hasattr(loader.dataset, 'data'):
            ...
            if hasattr(loader.dataset, 'targets') and loader.dataset.targets is not None:
