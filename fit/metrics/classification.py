...
def classification_report(
    ...
    report_lines = []
    report_lines.append(
        f"{'':>12} {'precision':>9} {'recall':>9} {'f1-score':>9} {'support':>9}"
    )
    report_lines.append("")
    ...
    for i, (name, precision, recall, f1, support) in enumerate(
        zip(target_names, precisions, recalls, f1s, supports)
    ):
        report_lines.append(
...
def roc_auc_score(
    ...
    if len(classes) == 2:
        ...
        if multi_class == "raise":
            raise ValueError(
                "ROC AUC is not defined for multiclass without specifying multi_class parameter"
            )
...
def log_loss(
    y_true: np.ndarray, y_pred: np.ndarray, eps: float = 1e-15, normalize: bool = True
) -> float:
    """
