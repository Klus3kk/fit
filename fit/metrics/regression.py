...
def mean_tweedie_deviance(
    y_true: np.ndarray, y_pred: np.ndarray, power: float = 0
) -> float:
    """
...
def get_regression_metrics(
    ...
    try:
        ...
        metrics["concordance_corr"] = concordance_correlation_coefficient(
            y_true, y_pred
        )
    except:
        pass
...
def regression_error_analysis(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, Any]:
    ...
    analysis["residual_skewness"] = np.mean(
        ((residuals - np.mean(residuals)) / np.std(residuals)) ** 3
    )
    analysis["residual_kurtosis"] = (
        np.mean(((residuals - np.mean(residuals)) / np.std(residuals)) ** 4) - 3
    )
