...
class StratifiedKFoldCV:
    ...
    def split(
        ...
        unique_classes, y_indices, class_counts = np.unique(
        ...
        for fold_id in range(self.n_splits):
            ...
            for class_id, (class_idx, assignments) in enumerate(
                zip(class_indices, fold_assignments)
            ):
...
class GroupKFoldCV:
    ...
    def split(
        ...
        for fold_id in range(self.n_splits):
            test_groups = [
                group for group, fold in group_to_fold.items() if fold == fold_id
            ]
...
def cross_validate(
    ...
    if hasattr(cv_splitter, "split"):
        if (
            groups is not None
            and hasattr(cv_splitter, "split")
            and "groups" in cv_splitter.split.__code__.co_varnames
        ):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(splits):
        ...
        if hasattr(model_clone, "fit"):
            ...
            if return_train_score:
                ...
                if (
                    y_train_pred_tensor.data.ndim > 1
                    and y_train_pred_tensor.data.shape[1] > 1
                ):
    ...
    if stratify is not None:
        ...
        for cls in unique_classes:
            ...
            train_indices.extend(
                cls_indices[
                    cls_n_test : cls_n_test + int(cls_n_samples * (n_train / n_samples))
                ]
            )
...
def kaggle_time_series_split(
    df: np.ndarray, time_column_idx: int, test_size: float = 0.2, gap_size: int = 0
...
def kaggle_group_split(
    X: np.ndarray, groups: np.ndarray, test_size: float = 0.2
) -> Tuple[np.ndarray, np.ndarray]:
    """
