...
class RFE:
    ...
    def __init__(self, estimator, n_features_to_select: Optional[int] = None, step: int = 1):
        """
        Initialize RFE.
    ...
    for feature_idx in range(n_features):
        ...
        for cls in unique_classes:
            ...
            if len(class_data) > 0:
                ...
                group_vars.append(np.var(class_data, ddof=1) if len(class_data) > 1 else 0)
...
def f_regression(X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for feature_idx in range(n_features):
        ...
        denominator = np.sqrt(np.sum((feature_data - feature_mean)**2) * 
                             np.sum((y - y_mean)**2))
...
def mutual_info_classif(X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for feature_idx in range(n_features):
        ...
        if n_bins > 1:
            # Simple binning
            feature_binned = np.digitize(feature_data, 
                                       np.linspace(feature_data.min(), 
                                                 feature_data.max(), n_bins))
